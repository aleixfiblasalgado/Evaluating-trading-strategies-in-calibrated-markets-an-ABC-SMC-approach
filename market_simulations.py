...
def rmsc03_4(parameters, initial_cash = 1000000):
    ...
    try:
        _, stream_df, orderbook_df, _ = kernel.runner(agents=agents,
        ...
        del stream_df, orderbook_df, ohlcv, stream_processed, processed_ob, obs, obs_dict, keys, interarrival_times, trades_distribution
...
def AAPL_10(parameters, initial_cash = 1000000):
    ...
    r_bar = 16164
    sigma_n = r_bar / 10
    kappa = parameters["kappa"] # 1.67e-16
    lambda_a = 7e-11
    sigma_s = 0
    fund_vol = parameters["fund_vol"] # 1e-8
    megashock_lambda_a = parameters["megashock_lambda_a"] # 2.77778e-18
    megashock_mean = parameters["megashock_mean"] # 1e3
    megashock_var = parameters["megashock_var"] # 5e4
    ...
    symbols = {symbol: {'r_bar': r_bar,
                    ...
                    'megashock_mean': megashock_mean,
                    'megashock_var': megashock_var,
                    'random_state': np.random.RandomState(seed=np.random.randint(low=0, high=2 ** 25, dtype='uint64'))}}
    ...
    agents.extend([AdaptiveMarketMakerAgent(id=j,
                                ...
                                backstop_quantity=mm_backstop_quantity,
                                log_orders=log_orders,
                                random_state=np.random.RandomState(seed=np.random.randint(low=0, high=2 ** 25,
                                                                                          dtype='uint64')))
               for idx, j in enumerate(range(agent_count, agent_count + num_mm_agents))])
    agent_count += num_mm_agents
    agent_types.extend('POVMarketMakerAgent')
    ...
    pov_agent = POVExecutionAgent(id=agent_count,
                              ...
                              trade=trade,
                              log_orders=log_orders,  # needed for plots so conflicts with others
                              random_state=np.random.RandomState(seed=np.random.randint(low=0, high=2 ** 28,
                                                                                          dtype='uint64')))
    execution_agents = [pov_agent]
    agents.extend(execution_agents)
    agent_types.extend("ExecutionAgent")
...
def exp_agent(parameters, seed, initial_cash = 1000000):
    ...
    experimental_agent = ExampleExperimentalAgent(
        ...
        wake_freq='10s',
        order_size=100,
        short_window=str(parameters['ea_short_window'])+'s',
        long_window=str(parameters['ea_long_window'])+'s',
        log_orders=True,
        random_state=np.random.RandomState(seed=np.random.randint(low=0, high=2 ** 32, dtype='uint64'))
        )
    ...
    try:
        _, _, _, surplus = kernel.runner(agents=agents,
                      startTime=kernelStartTime,
                      stopTime=kernelStopTime,
                      agentLatencyModel=latency_model,
                      defaultComputationDelay=defaultComputationDelay,
