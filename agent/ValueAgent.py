...
class ValueAgent(TradingAgent):
    ...
    def __init__(self, id, name, type, symbol='IBM', starting_cash=100000, sigma_n=10000,
        ...
        self.maxint = 18446744073709551615
    ...
    def placeOrder(self):
        ...
        if bid and ask:
            ...
            if np.random.rand() < self.percent_aggr:
                ...
                if self.depth_spread*spread > self.maxint:
                    adjust_int = np.random.randint( 0, self.maxint, dtype='uint64')
                    
                elif self.depth_spread*spread <= 0:
                    adjust_int = 0
                    
                else:
                    adjust_int = np.random.randint( 0, self.depth_spread*spread, dtype='uint64')
