...
class Kernel:
  ...
  def __init__(self, kernel_name, parameters, random_state = None):
  ...
  def runner(self, agents = [], startTime = None, stopTime = None,
    ...
    for sim in range(num_simulations):
      ...
      for agent in agents:
        if type(agent).__name__ == 'ExchangeAgent':
              stream_df, orderbook_df = agent.kernelTerminating()
        else:
            agent.kernelTerminating()
    ...
    # log itself.
    summary_log = pd.DataFrame(self.summaryLog)
    ...
    for a in self.meanResultByAgentType:
      surplus = None
      if a ==   "ExampleExperimentalAgent":
          surplus = self.meanResultByAgentType[a]
    ...
    return self.custom_state, stream_df, orderbook_df, surplus
