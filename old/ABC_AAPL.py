...
if __name__ == "__main__":
    ...
    prior = pyabc.Distribution(
        num_value_ag=pyabc.RV('randint', 10, 300),
    ...
    abc = pyabc.ABCSMC(rmsc03, prior, distance,  population_size=pop_size, 
                       transitions=transition, sampler = SingleCoreSampler())
    db_path = os.path.join(os.getcwd()+'/ABC_output', "proof_concept.db")

    abc.new("sqlite:///" + db_path, data)
    
    epsilon = 0.05
    
    history1 = abc.run(max_nr_populations=3, minimum_epsilon=epsilon)
