...
if __name__ == "__main__":
    ...
    mapping={
        'num_value_ag': pyabc.DiscreteJumpTransition(
            domain=np.arange(20, 501), p_stay=0.3
        ),
        'num_momentum_ag': pyabc.DiscreteJumpTransition(
            domain=np.arange(20, 301), p_stay=0.3
        ),
        'num_noise_ag': pyabc.DiscreteJumpTransition(
            domain=np.arange(2000, 7001), p_stay=0.3
        ),
    ...
    abc.new(pyabc.create_sqlite_db_id(dir_= db_path, file_='experiment_1.db'), data)
