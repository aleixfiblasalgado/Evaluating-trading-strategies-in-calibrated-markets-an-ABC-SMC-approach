...
from market_simulations_timed import rmsc03_4_t as market
...
if __name__ == "__main__":
    ...
    prior = pyabc.Distribution(
        num_value_ag=pyabc.RV('randint', 20, 500),
        num_momentum_ag=pyabc.RV('randint', 20, 300),
        num_noise_ag=pyabc.RV('randint', 2000, 7000),
    ...
    mapping={
        'num_value_ag': pyabc.DiscreteJumpTransition(
            domain=np.arange(20, 501), p_stay=0.4
        ),
        'num_momentum_ag': pyabc.DiscreteJumpTransition(
            domain=np.arange(20, 301), p_stay=0.4
        ),
        'num_noise_ag': pyabc.DiscreteJumpTransition(
            domain=np.arange(2000, 7001), p_stay=0.4
        ),
    ...
    abc.new(pyabc.create_sqlite_db_id(dir_= db_path, file_='experiment_1_war.db'), data)
    
    abc.run(max_nr_populations=6, minimum_epsilon=epsilon)