...
def get_trades(stream_file):
  
  df = stream_file[stream_file['EventType'] == 'LAST_TRADE']
  if len(df) <= 0:
    print("There appear to be no simulated trades.")
    sys.exit()
  df = df.assign(PRICE = [y for x,y in df['Event'].str.split(',')])
  df = df.assign(SIZE = [x for x,y in df['Event'].str.split(',')])
  df = df.assign(PRICE = df['PRICE'].str.replace('$','').astype('float64'))
  df = df.assign(SIZE = df['SIZE'].astype('float64'))
...
def get_orderbook_stylized_facts(orderbook_df, ob_processed, stream_processed, num_levels=5, order_type="executed",
                                 freq="20S"):
    ...
    *[[f'ask_price_{level}', f'ask_size_{level}', f'bid_price_{level}', f'bid_size_{level}'] for level in
    ...
    transacted_orders = merged.loc[merged.TYPE == "ORDER_EXECUTED"]
    limit_orders = merged.loc[merged.TYPE == "LIMIT_ORDER"]
    ...
    if order_type=="executed":
        ...
        ov_imbalance_w = (limit_orders['ask_size_1'].sum()/(limit_orders['ask_size_1'].sum() + limit_orders['bid_size_1'].sum())).ffill()
