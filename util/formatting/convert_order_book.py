...
def process_row(row, quote_levels):
    ...
    ## reduce size of the orderbook for further processing
    if num_asks > 40:
        ask_values = ask_values[0:40]
        ask_volumes = ask_volumes[0:40]
        num_asks = ask_values.size
        
    if num_bids > 40:
        bid_values = bid_values[0:40]
        bid_volumes = bid_volumes[0:40]
        num_bids = bid_values.size
